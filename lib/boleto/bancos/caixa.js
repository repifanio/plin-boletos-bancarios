...
const CodBarrasCaixa = require('../codigo-barras-caixa');
...
var Caixa = (function() {
	...
	Caixa.prototype.geraCodigoDeBarrasPara = function(boleto) {
		...
		const valorBoleto = boleto._valorBoleto.replace('.', '');
		const banco = '104';

		const codBarras = new CodBarrasCaixa();
		const fatorVencimento = codBarras.calcFatorVencimento(boleto._datas._vencimento);
		const campoLivre2= codBarras.calcCampoLivre(boleto._beneficiario._nrCedente, `${boleto._beneficiario._carteira}${boleto._beneficiario._nossoNumero}`);
		const dvCampoLivre = codBarras.calcMod11DvCampoLivre(campoLivre2);

		const concat = `${banco}${boleto._codigoEspecieMoeda}${fatorVencimento}${valorBoleto.padStart(10, '0')}${campoLivre2}${dvCampoLivre}`;

		const dvGeral = true;
		const dvGeralCodBarras = codBarras.calcMod11DvCampoLivre(`${banco}${concat}`, dvGeral);


		const codigoBarrasCompleto = `${banco}${boleto._codigoEspecieMoeda}${dvGeralCodBarras}${fatorVencimento}${valorBoleto.padStart(10, '0')}${campoLivre2}${dvCampoLivre}`;
