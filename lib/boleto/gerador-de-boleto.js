	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(`${beneficiario._agencia}/${beneficiario._nrCedente}`, args.ajusteX + 32, args.ajusteY + segundaLinha, {
						lineBreak: false,
						width: 294,
						align: 'left'
					});
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(`${beneficiario._agencia}/${beneficiario._nrCedente}`, args.ajusteX + colunaLateral, args.ajusteY + quartaLinha, {
						lineBreak: false,
						width: tamanhoDasCelulasADireita,
						align: 'right'
					});
